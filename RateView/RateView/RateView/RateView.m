...
- (void)refresh {
    for(int i = 0; i < self.imageViews.count; ++i) {
        ...
        if (self.rating >= i+1) {
            ...
            if (_hideUnRateImage == YES) {
                imageView.hidden = _hideUnRateImage;
            }
        }
    }
}
...
- (void)handleTouchAtLocation:(CGPoint)touchLocation {
    ...
    for(int i = self.imageViews.count - 1; i >= 0; i--) {
        UIImageView *imageView = [self.imageViews objectAtIndex:i];        
        if (touchLocation.x > imageView.frame.origin.x && touchLocation.x < imageView.frame.origin.x + 20) {
            newRating = i + 0.5;
            break;
        } else if (touchLocation.x > imageView.frame.origin.x + 20) {
            newRating = i + 1;
            break;
        }
    }
